...
test("requires private share token for live room writes", async () => {
  ...
  try {
    ...
    const updates = [];
    const ended = [];
    viewer.on("location-update", (update) => updates.push(update));
    viewer.on("sharing-ended", () => ended.push(true));
    ...
    const stopped = await fetch(`${ORIGIN}/api/rooms/${created.roomId}`);
    assert.deepEqual(await stopped.json(), { exists: false });
