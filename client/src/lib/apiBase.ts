 ...
 * - En build de Vercel, **Socket.io va directo a Render** (`VITE_DEFAULT_RENDER_BACKEND`): el proxy WebSocket de
 *   Vercel → origen externo suele fallar (“websocket error”) y el visor no recibe `location-update`.
 * - En build servido por Render, Socket.io queda en el mismo origen para hablar con el mismo proceso que creó la sala.
...
/**
 * Origen para Socket.io: Vercel usa Render directo; Render/dev/preview local usan el mismo origen (`undefined`).
 */
