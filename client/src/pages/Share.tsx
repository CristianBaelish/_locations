...
export function Share() {
  ...
  const shareToken = shareTokenFromState(location.state) ?? readShareToken(roomId);
  const missingShareToken = !!roomId && !shareToken;
  const viewerUrl =
  ...
  return (
    <div className="layout">
      ...
      {missingShareToken ? (
        <p style={{ color: "var(--danger)", marginBottom: "1rem" }} role="alert">
          Esta pestaña no tiene permiso para compartir esta sesión. Empezá una sesión nueva.
        </p>
      ) : null}
